...
class API:
    ...
    def __init__(
        ...
        wait_on_rate_limit=False, follows_remainder=1
    ):
        self.auth = auth
        self.cache = cache
        self.host = host

        self.follows_remainder = follows_remainder
        self.user_remainder = 885
        self.divider = 30
        ...
        try:
            ...
            while retries_performed <= self.retry_count:
                ...
                try:
                    ...
                    rlimit = int(resp.headers.get('x-rate-limit-remaining'))
                    rtime = int(resp.headers.get('x-rate-limit-reset'))
                    if rlimit < 50:
                        self.divider = rlimit * 2
                    if rlimit < self.follows_remainder:
                        sleep_time = rtime - int(time.time())
                        log.warning(f'Rate limit reached. Sleeping for {sleep_time}')
                        time.sleep(sleep_time + 1)  # Sleep for extra sec
                        self.user_remainder = 885
                        self.divider = 30
