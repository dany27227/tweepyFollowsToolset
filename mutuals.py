...
if __name__ == '__main__':
    ...
    users = ['leighalexander', 'BootsRiley', 'pitchfork']

    for x in range(len(users) - 1):
        if x == 0:
            set1 = followsIO.loadFollows(mode='usr', file=users[x])
            set2 = followsIO.loadFollows(mode='usr', file=users[x+1])

            matches = set1.intersection(set2)
            headerString = f'{users[x]}: {len(set1)}//////{users[x+1]}: {len(set2)}//////'
        else:
            newSet = followsIO.loadFollows(mode='usr', file=users[x+1])
